...
int SciTEBase::IsMenuItemEnabled(int cmd) {
	...
	{
		for (int i = 0; i < buffers.length; i++) {
			if (buffers.buffers[i].DocumentNotSaved()) return 1;
		}
		return 0;
	}
	break;
	case IDM_SAVE:
		return CurrentBuffer()->DocumentNotSaved() ? 1 : 0;
		break;
	case IDM_UNDO:
		return CallFocused(SA::Message::CanUndo) ? 1 : 0;
		break;
	case IDM_REDO:
		return CallFocused(SA::Message::CanRedo) ? 1 : 0;
		break;
	case IDM_DUPLICATE:
		return (!CurrentBuffer()->isReadOnly) ? 1 : 0;
		break;
	case IDM_FINDINFILES:
	...
	case IDM_COPY:
	case IDM_CLEAR:
		return (CallFocused(SA::Message::GetSelectionStart) != CallFocused(SA::Message::GetSelectionEnd)) ? 1 : 0;
		break;
	case IDM_PASTE:
		return CallFocused(SA::Message::CanPaste) ? 1 : 0;
		break;
	case IDM_OPENFILESHERE:
		return props.GetInt("check.if.already.open") ? 1 : 0;
		break;
	case IDM_COMPILE:
		return (!jobQueue.IsExecuting() && props.GetWild("command.compile.", FileNameExt().AsUTF8()).size()) ? 1 : 0;
		break;
	case IDM_BUILD:
		return (!jobQueue.IsExecuting() && props.GetWild("command.build.", FileNameExt().AsUTF8()).size()) ? 1 : 0;
		break;
	case IDM_GO:
		return (!jobQueue.IsExecuting() && props.GetWild("command.go.", FileNameExt().AsUTF8()).size()) ? 1 : 0;
		break;
	case IDM_OPENDIRECTORYPROPERTIES:
		return props.GetInt("properties.directory.enable") ? 1 : 0;
		break;
	case IDM_STOPEXECUTE:
		return jobQueue.IsExecuting() ? 1 : 0;
		break;
