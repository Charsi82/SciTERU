...
namespace {
...
#ifdef RB_UTF8AC
		void set_utf8_autocheck(bool) noexcept override;
#endif // RB_UTF8AC
...
#ifdef RB_UTF8AC
		int m_nAutoCheckUtf8 = 0;//!-add-[utf8.auto.check]
#endif
	};
...
#ifdef RB_UTF8AC
	void Utf8_16_Reader::set_utf8_autocheck(bool arg) noexcept {
		m_nAutoCheckUtf8 = arg;
	}
#endif
	...
	int Utf8_16_Reader::determineEncoding(std::string_view text) noexcept {
		...
		if (text.length() > 1) {
			...
			else if (text.length() > 2 && m_nAutoCheckUtf8 == 1) {
				if (Has_UTF8_Char(text.data(), text.length())) {
					m_eEncoding = UniMode::utf8;
					nRet = 0;
				}
			}
...
bool Has_UTF8_Char(const char* buf, size_t size) noexcept {
	...
	while (p[0] && i < size) {
		if (p[0] >> 7 == 0x01) {
			if (p[0] >> 5 == 0x06 && (i + 1) < size) {
				...
				if (p[2] >> 6 != 0x02) { return false; } //not utf8
				if (p[3] >> 6 != 0x02) { return false; } //not utf8
				if (p[4] >> 6 != 0x02) { return false; } //not utf8
				...
				if (p[3] >> 6 != 0x02) { return false; } //not utf8
				if (p[4] >> 6 != 0x02) { return false; } //not utf8
				if (p[5] >> 6 != 0x02) { return false; } //not utf8
