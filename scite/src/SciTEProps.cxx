...
void SciTEBase::ReadGlobalPropFile() {
	...
	props.Set("SciteDefaultHome", homepath.AsUTF8());
	homepath = GetSciteUserHome();
	props.Set("SciteUserHome", homepath.AsUTF8());
	//!-end-[scite.userhome]
#endif // RB_SUH
...
#ifdef RB_CARET
	//!-start-[caret]
	std::string tmp_str = props.GetNewExpandString("caret.fore.", fileNameForExtension);
...
#ifdef RB_CARET
	//!-start-[caret]
	tmp_str = props.GetNewExpandString("caret.width.", fileNameForExtension);
	if (tmp_str.length()) {
		wEditor.SetCaretWidth(IntFromHexByte(tmp_str));
		wOutput.SetCaretWidth(IntFromHexByte(tmp_str));
	}
	else {
#endif // RB_CARET
...
#ifdef RB_GWC
	//!-start-[GetWordChars]
	props.Set("CurrentWordCharacters", wordCharacters);
	//!-end-[GetWordChars]
#endif // RB_GWC
