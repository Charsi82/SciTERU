...
namespace {
	...
	int impl_scite_lua_rename(lua_State* L) {
...
namespace {
	...
	int lua_string_utf8len(lua_State* L) {
...
namespace {
	...
	int utf8_loader_Lua(lua_State* L) {
		const char* filename;
	...
	int utf8_loader_Croot(lua_State* L) {
		const char* funcname;
		const char* filename;
		const char* name = luaL_checkstring(L, 1);
...
namespace {
	...
	int utf8_searcher_Lua(lua_State* L)
	{
		const char* filename;
		const char* name = luaL_checkstring(L, 1);
	...
	void* checkclib(lua_State* L, const char* path) {
		void* plib;
	...
	void addtoclib(lua_State* L, const char* path, void* plib) {
		lua_getfield(L, LUA_REGISTRYINDEX, CLIBS);
	...
	int lookforfunc(lua_State* L, const char* path, const char* sym) {
	...
	int loadfunc(lua_State* L, const char* filename, const char* modname) {
		const char* openfunc;
		const char* mark;
	...
	int utf8_searcher_C(lua_State* L)
	{
		const char* name = luaL_checkstring(L, 1);
	...
	int utf8_searcher_Croot(lua_State* L)
	{
		const char* filename;
		const char* name = luaL_checkstring(L, 1);
		const char* p = strchr(name, '.');
