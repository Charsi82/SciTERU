	...
	for (items = 0; items < toolMax; items++) {
		...
		if (sMenuItem.length()) {
			...
			if (items < 10 && sMnemonic.empty())
				sMnemonic += "Ctrl+" + sCmdID;
			prefix = "command.separator." + sCmdID + ".";
			std::string val = props.GetNewExpandString(prefix, sFNE);
			int issep = IntegerFromString(val, 0);
			prefix = "command.parent." + sCmdID + ".";
			val = props.GetNewExpandString(prefix, sFNE);
			int toMenu = IntegerFromString(val, 0);
			GUI::gui_string lsMenuItem = localiser.Text(sMenuItem);
			prefix = "command.checked." + sCmdID + ".";
			val = props.GetNewExpandString(prefix, sFNE);
	...
	for (items = 1; items < toolMax; items++) {
		if (arrMenu[items].GetID() != 0) {
			...
			if (commandName.length()) {
				...
				std::string val = props.GetNewExpandString(prefix, FileNameExt().AsUTF8());
				int toMenu = IntegerFromString(val, 0);
				GUI::gui_string lcommandName = localiser.Text(commandName);
				if (toMenu > 0 && toMenu < toolMax) {
