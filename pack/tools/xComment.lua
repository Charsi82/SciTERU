...
--[[	local pat = ""
	...
	return pat]]
	return text:pattern() -- using COMMON.lua
end
...
-- Определение что выделено - потоковый комментарий или закомментированные строки
	if PatternStream ~= "" then
		if lexer == 'lua' then
			if string.find(sel_text,"%-%-%[(=*)%[.+%]%1%]") then
				if iDEBUG then print ("This Lua Stream Comment") end
				return true
			end
		end
...
-- 		local line_pos_end = line_pos_start + string.len(text_line)
		local line_pos_end = editor.LineEndPosition[line_sel_start]
		local eol = text_line:match("([\r\n]?[\r\n])$")
		if eol then line_pos_end = line_pos_end + #eol end
...
--------------------------------------------------
local function luafix_StreamComment()
	if string.find(sel_text, comment_stream_end) then
		comment_stream_end = comment_stream_end:gsub("%]$","=]")
		comment_stream_start = comment_stream_start:gsub("%[$","=[")
		luafix_StreamComment()
	end
end
...
-- Комментирование выделенного потока текста
	...
	if comment_stream_start == "" or comment_stream_end == "" then
		...
		if lexer == 'lua' then
			-- extend comment_stream if sel_text content long string
			luafix_StreamComment()
			-- add space if sel_text endging for ']'
			if sel_text:match("%]$") then comment_stream_end = " "..comment_stream_end end
		end
...
--------------------------------------------------
local function luafix_StreamUnComment()
	comment_stream_start, equ = sel_text:match("(%-%-%[(=*)%[)")
	comment_stream_end = "]"..equ.."]"
...
-- Снятие комментария с выделенного потока текста
	...
	if PatternStream == "" then
		...
		if lexer == 'lua' then luafix_StreamUnComment() end
