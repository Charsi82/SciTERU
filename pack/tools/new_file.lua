...
AddEventHandler("OnBeforeSave", function(file)
	if isMakeUTF8() and tonumber(props["editor.unicode.mode"]) == IDM_ENCODING_DEFAULT then
		...
		local new_text = txt_in:to_utf8(editor:codepage())
		-- save bookmarks
		local bms = {}
		local line = 0
		while true do
		...
		-- save caret pos
		local caret_pos = editor.CurrentPos + new_text:len() - txt_in:len()
		...
		-- restore caret pos
		editor:GotoPos(caret_pos)
		
		-- restore bookmarks
		for _, line in ipairs (bms) do
