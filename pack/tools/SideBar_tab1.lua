...
return function(tabs, panel_width, colorback, colorfore)
	...
	do
		...
		do -- v------- Lua -------v--
			...
			-- Паттерн для длинных скобок: [[ ... ]] или [=[ ... ]=] и т. д.
			local function make_long_brackets()
				local level = P "=" ^ 0 -- захватываем уровень (количество '=')
				local open = P "[" * level * P "["
				local close = P "]" * level * P "]"
				return open * (1 - close) ^ 0 * close
			end

			local LUALONGSTR = make_long_brackets()
            ...
            -- obj.method, server.api.validate и т.п.
            -- obj:method, string:upper и т.п.
            local FN_IDENTIFIER = IDENTIFIER * (P '.' * IDENTIFIER) ^ 0 *
                                      (P ':' * IDENTIFIER) ^ -1

			-- create additional captures
            local I = C(FN_IDENTIFIER) * cl
			-- definitions to capture:
			...
			local funcdef2 = l * I * SC ^ 0 * "=" * SC ^ 0 * f * SC ^ 0 * par -- declaration through assignment
             ...
             local unique_anon = function(pos)
					fcounter = fcounter + 1
					return "func_#" .. fcounter
				end

            local make_anon_body = P(function(subject, pos)
                local depth = 1 -- 1, потому что мы уже внутри вызова: call(function ...
                local i = pos
                local len = #subject
                ...
                while i <= len do
                    ...
                    elseif c == ')' then
                        depth = depth - 1
                        if depth == 0 then
                            -- Закрылась скобка вызова — это НЕ функция
                            return -- отвергаем паттерн
                        end
                        i = i + 1
                    elseif c == 'e' and subject:sub(i, i + 2) == 'end' then
                        local after =
                            i + 3 <= len and subject:sub(i + 3, i + 3) or ''
                        if not after:match('[%w_]') then -- слово "end"
                            if depth == 1 then
                                -- Это `end` функции, и мы на уровне функции
                                return i + 3 -- позиция после `end`
                            else
                                -- `end` внутри вложенного выражения
                                i = i + 3
                            end
                        else
                            i = i + 1
            ...
            -- анонимная функция в вызове функции
            local funcdef_anon = f * cp / unique_anon * cl * SC ^ 0 * par * SC ^ 0  * make_anon_body

            local afn = tonumber(props['sidebar.anonimous.function']) or 0 ~= 0 -- включает парсинг анонимных функий
            local def = afn and Ct(funcdef1 + funcdef2 + funcdef_anon) or
                            Ct(funcdef1 + funcdef2)
			-- resulting pattern, which does the work
		...
		do -- v------- CSS -------v--
			-- helper
			local function clear_spaces(s) return s:gsub('%s+', ' ') end
			-- redefine common patterns
			local IDENTIFIER = (ANY - SPACE) ^ 1
			local NL = P "\r\n"
			local SPACE = S ' \t' ^ 1
	...
	local function Functions_ListFILL()
		...
		if _sort == 'order' then
            table.sort(table_functions, function(a, b)
                return a[2] < b[2]
            end)
		else
            table.sort(table_functions,
                       function(a, b)
        ...
        local function fixname(funcitem)
			...
			local ret = funcitem[1]
			if _show_flags then ret = GetFlags(funcitem) .. ret end
			if _show_params then ret = ret .. GetParams(funcitem) end
            return ret 
        end
	...
	local function UpdateListMenu()
		if list_mnu then
			list_mnu:check_item(1, _show_flags)
			list_mnu:check_item(2, _show_params)
			local sm = list_mnu:get_submenu(0)
			if sm then
				sm:check_item(0, _sort == 'order')
				sm:check_item(1, _sort == 'name')
			end
		end
	end
	...
	list_func:on_double_click(function() Functions_GotoLine() end)
	...
	local function get_or_add_parent_item()
		...
		if not res then
            ...
            res = tree_bookmarks:add_item(props['FileNameExt'], nil,
                                          BM_FILE_IDX, BM_FILE_IDX,
                                          props['FilePath'])
	...
	AddEventHandler("OnInit", function()
		for _, t in ipairs(RecentData) do
			...
			if shell.fileexists(full_path) and #t.bookmarks > 0 then
                local parent_item = tree_bookmarks:add_item(full_path:match(
                                                                '[^\\]+$'), nil,
                                                            BM_FILE_IDX,
                                                            BM_FILE_IDX,
                                                            full_path)
                for _, bm_line in ipairs(t.bookmarks) do
                    tree_bookmarks:add_item(bm_line.caption, parent_item,
                                            BM_LINE_IDX)
                end
			end
		end
	...
	-- update bookmark on save
	AddEventHandler("OnSave", function(file)
		local bm_count = BookmarksCount()
		if bm_count>0 then
	...
	local function Func2Line(funcname)
		if not next(table_functions) then Functions_GetNames() end
	...
	AddEventHandler("OnDoubleClick", function(shift, ctrl, alt)
		...
		if shift then return JumpToFuncDefinition() end
	end)
