...
if str:find('https?://(.*)') then
	shell.exec(str)
elseif shell.fileexists(str) then -- is name of file or folder
	if #gui.files(str, true)>0 then -- is folder
		shell.exec(str)
	else
		shell.exec("explorer /select,"..str)
	end
else
	...
	print('>> '..scite.GetTranslation("Result")..': '..(result or ''))
