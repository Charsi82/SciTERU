...
local curr_lang = ''   -- лексер активной вкладки
...
AddEventHandler("OnUpdateUI", function()
	if props['FileName'] ~= '' then
		if tonumber(props["highlighting.identical.text"]) == 1 then
			if props['Language'] ~= curr_lang then
				curr_lang = props['Language']
				reserved_words = props['highlighting.identical.text.reserved.words.' .. curr_lang]
