...
return function(tabs, panel_width, colorback, colorfore)
	...
	function FileMan_FileCopy()
		...
		for _,i in ipairs(si) do
			...
			if dir_or_file ~= '..' then
				os_copy(current_path..dir_or_file, path_destination)
			end
		end
		FileMan_ListFILL()
	end
	...
	function FileMan_FileMove()
		...
		for _,i in ipairs(si) do
			local dir_or_file, attr = FileMan_GetSelectedItem(i)
			if dir_or_file ~= '..' then
				os_move(current_path..dir_or_file, path_destination..'\\'..dir_or_file)
			end
		end
		FileMan_ListFILL()
	end
	...
	function FileMan_FileDelete()
		...
		for _,i in ipairs(si) do
			local dir_or_file, attr = FileMan_GetSelectedItem(i)
			if attr ~= 'd' then
				table.insert(files, current_path..dir_or_file)
			end
		end
	...
	local function crc_enum_selection(crc_type)
		...
		for _,i in ipairs(si) do
			local dir_or_file, attr = FileMan_GetSelectedItem(i)
			if attr ~= 'd' then
				calc_crc(current_path..dir_or_file , crc_type)
			end
		end
	end
	
	function FileMan_CalcMD5()
		crc_enum_selection('md5')
	end
	function FileMan_CalcSHA1()
		crc_enum_selection('sha1')
	end
	function FileMan_CalcSHA256()
		crc_enum_selection('sha256')
	end
	function FileMan_CalcSHA512()
		crc_enum_selection('sha512')
	end
