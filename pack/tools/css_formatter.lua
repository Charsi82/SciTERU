...
Version: 1.2
------------------------------------------------------
Приводит выделенную таблицу стилей к виду:
tag {
	prop: value;	/* comment */
	prop: value;	/* comment */
}
Параметры css.format.sort.tags и css.format.sort.props определяют
...
if tonumber(props['css.format.sort.tags']) then
	table.sort(t_style, function(a, b)
						...
						local _a = a['tag']:gsub('^#', 'ю'):gsub('^%.', 'я')
						local _b = b['tag']:gsub('^#', 'ю'):gsub('^%.', 'я')
