...
3.0 (Tymur):
	...
	* попутно выполнил задачи 1 и 2 из списка выше
	+ добавил фичу для Lua: распознавание строковых переменных в коде (ассоциируются с таблицей string)
	...
	* исправил старый баг, описанный в сообщении VoidVolker: http://forum.ru-board.com/topic.cgi?forum=5&topic=24956&start=840#5
	Проверено только для Lua, пока что работает :)
	+ если в api-файле Lua встречаются строчки вида: t["a-b+c"]\nэто очень хитрый параметр,
	  то имеет смысл добавить в autocomplete.lua.start.characters символ '['
3.01(Tymur):
	+ скрипт опять нечувствительный к регистру (см. "-" версии 3.0)
	+ автоматически исправляется регистр символов введенного имени объекта (слово слева от разделителя) на указанный в api-файле (для Lua: StriNG станет string), но только при использовании автодополнения.
3.05(Tymur):
...
3.09(Tymur):
	...
	+ теперь в .api файлах допустима конструкция вида #$string_value=^'.*' и даже #$file=io.open%b(). Часть справа от знака "=" -- паттерн, используемый для распознавания объекта. Т.о. возможно расширить функциональность фичи для распознавания строк на всё, что ловится регулярными выражениями Lua.
...
local function GetInputObject()
	...
	local word_start_pos = editor:BraceMatch(current_pos-2,0)
...
local function CreateObjectsAndAliasTables()
	...
	for i = 1, #api_table do
		...
		-- здесь КРАЙНЕ ВАЖНО, чтобы в матче был именно [auto_start_chars_patt], т.е. например "[.:]" для Lua
...
-- 		editor:SetSel(current_pos, editor.CurrentPos)
		...
		-- поглощение одинаковых символов при вставке
		local endpos = editor.CurrentPos
		local txt_line, pos_in_line = editor:GetCurLine()
		txt_line = txt_line:sub(pos_in_line + 1) -- tail of current line
		local shift = 0
		local s1 = sel_value:lower()
		local s2 = txt_line:lower()
		for i=1,#s1 do
			if s1:sub(-i) == s2:sub(1,i) then
				shift=i
				break
			end
		end
