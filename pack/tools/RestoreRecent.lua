...
-- ON STARTUP SCITE --
----------------------
local buffers = {} -- массив {номер_файла, {имя_параметра, значение_параметра} }
...
-- 						editor:ToggleFold(line_num)
		...
		if tonumber(props['session.bookmarks']) == 1 then
			...
			if bookmarks ~= nil then
				for line_num in string.gmatch(bookmarks, "%d+") do
					local line = tonumber(line_num)-1
					if editor:MarkerGet(line, 2)//2%2~=1 then
		...
		if tonumber(props['save.position']) == 1 then
			local pos = tonumber(FileParams['position'])
			if pos then
...
AddEventHandler("OnFinalise", function()
	if props['FileName'] ~= '' then
		if tonumber(props['save.session.recent']) == 1 then
			...
			local cmd = 'wscript "'..script_dir:to_utf8(0)..'RestoreRecent.js" "'..props["SciteUserHome"]..'"'
