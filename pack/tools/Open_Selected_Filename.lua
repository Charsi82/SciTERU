...
local function FindFileDown(filename)
	...
	local function DIR(path)
		...
		if folders then for _, folder in pairs(folders) do DIR(path .. '\\' .. folder) end end
	end
	DIR(props['FileDir'])
	return findfile
end
...
local function GetOpenFilePath(text, shift)
	...
	if scite_var then return string.gsub(text, pattern_sci, props[scite_var]) end

	-- Example: %APPDATA%\Opera\Opera\profile\opera6.ini
	local pattern_env = '^[%%](.-)[%%]'
	local _, _, os_env = string.find(text, pattern_env)
	if os_env then return string.gsub(text, pattern_env, os.getenv(os_env)) end
	...
	if scite_prop1 then return props[scite_prop1] .. scite_prop2 end
...
local function GetSelText()
	...
	local text = pane:GetSelText():trim()
...
local function OpenSelectedFilename(text, shift)
	...
	if shell.fileexists(filename) then
		...
		if shell.msgbox(alert, scite.GetTranslation('Create New File'), 4 + 256) == 6 then
			...
			if not shell.fileexists(folder) then shell.exec('cmd /c md "' .. folder .. '"', nil, true, true) end
...
AddEventHandler("OnMenuCommand", function(msg, source) if msg == IDM_OPENSELECTED then return OpenSelectedFilename(GetSelText()) end end)

AddEventHandler("OnDoubleClick", function(shift, ctrl, alt) if ctrl and not alt and props["open.filename.by.click"] == "1" then return OpenSelectedFilename(GetClickedText(), shift) end end)
