...
local function OpenSelectedFilename(text, shift)
    ...
    if shell.fileexists(filename) then
        ...
        local alert =
            scite.GetTranslation('File') .. ' "' .. filename .. '" ' ..
        ...
        if shell.msgbox(alert, scite.GetTranslation('Create New File'), 4 + 256) == 6 then
            ...
            local warning_couldnotopenfile_disable =
                props['warning.couldnotopenfile.disable']
            props['warning.couldnotopenfile.disable'] = 1
            scite.Open(filename)
            props['warning.couldnotopenfile.disable'] =
                warning_couldnotopenfile_disable
        end
    end
    return true
end
...
AddEventHandler("OnMenuCommand", function(msg, source)
    if msg == IDM_OPENSELECTED then return OpenSelectedFilename(GetSelText()) end
end)
