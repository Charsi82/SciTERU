...
bool CSHA1::HashFile(const TCHAR* tszFileName)
{
	...
	FILE* fpIn;
	_tfopen_s(&fpIn, tszFileName, _T("rb"));
...
bool CSHA1::ReportHash(TCHAR* tszReport, REPORT_TYPE rtReportType) const
{
	...
	if((rtReportType == REPORT_TYPE::REPORT_HEX) || (rtReportType == REPORT_TYPE::REPORT_HEX_SHORT))
	{
		_sntprintf_s(tszTemp, 15, _T("%02X"), m_digest[0]);
		_tcscpy_s(tszReport, tszTemp);

		const TCHAR* lpFmt = ((rtReportType == REPORT_TYPE::REPORT_HEX) ? _T(" %02X") : _T("%02X"));
		for(size_t i = 1; i < 20; ++i)
		{
			_sntprintf_s(tszTemp, 15, lpFmt, m_digest[i]);
			_tcscat_s(tszReport, tszTemp);
		}
	}
	else if(rtReportType == REPORT_TYPE::REPORT_DIGIT)
	{
