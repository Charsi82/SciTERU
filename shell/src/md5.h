...
public:
	...
	{
		return digestStringL(string, static_cast<unsigned int>(strlen(string)));
	}

	char* digestStringL(const char* string, unsigned int len)
	{
		Init();
		Update(reinterpret_cast<unsigned char*>(const_cast<char*>(string)), len);
		Final();
