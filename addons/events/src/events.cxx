...
static int do_event(lua_State* L)
{
...
static int do_register(lua_State* L)
{
...
static int do_unregister(lua_State* L)
{
	luaL_checktype(L, 2, LUA_TFUNCTION);
	event_arg(L)->unreg();
	lua_settop(L, 1);
...
static int do_trigger(lua_State* L)
{
	return event_arg(L)->trigger() ? 1 : 0;
}

static int do_print(lua_State* L)
{
	event_arg(L)->print();
	lua_settop(L, 1);
	return 1;
...
static int do_stop(lua_State* L)
{
	event_arg(L)->stop();
	lua_settop(L, 1);
	return 1;
...
static int do_setRemove(lua_State* L)
{
...
static int do_setfp(lua_State* L)
{
...
static int do_clear(lua_State* L)
{
	event_arg(L)->clear();
	lua_settop(L, 1);
	return 1;
...
static int do_name(lua_State* L)
{
	lua_pushstring(L, event_arg(L)->get_name());
	return 1;
}

static int do_tostring(lua_State* L)
{
...
extern "C"
LUALIB_API int luaopen_events(lua_State * L)
{
