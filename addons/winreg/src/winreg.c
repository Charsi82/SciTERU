...
REGSAM reg_aux_getaccess(lua_State *L, int i, REGSAM def){
	REGSAM acc = 0;
	if(lua_isnumber(L, i) || lua_isnoneornil(L, i)){
		acc = lua_optint(L, i, def);
	}else{
...
REGSAM reg_aux_getaccess2(lua_State* L, int i) {
	return reg_aux_getaccess(L, i, KEY_READ);
}
...
int reglib_createkey(lua_State *L){//reglib.createkey
	...
	REGSAM Access = reg_aux_getaccess(L, 2, KEY_WRITE);
...
int reglib_openkey(lua_State *L){//reglib.openkey
	...
	LONG ret = 0;
	const char * szPath = lua_checkstring(L, 1);
	REGSAM Access = reg_aux_getaccess2(L, 2);
...
BOOL reg_aux_setvalue(lua_State *L, HKEY hKey, const TCHAR * pszVal, int type, int i){
	...
	break; case REG_MULTI_SZ:{
		...
		if(lua_istable(L, i)){
			int n;
			lua_Integer last = lua_objlen(L, i);
			for (n = 1; n <= last; n++) {
				lua_rawgeti(L, i, n);
...
int reg_createkey(lua_State *L){//regobj.createkey
	reg_aux_newkey(L, reg_aux_gethkey(L, 1),
		lua_checktstring(L, 2), (PTSTR)lua_opttstring(L, 4, NULL), reg_aux_getaccess(L, 3, KEY_WRITE), TRUE);
	return 1;
}
//docok
...
int reg_openkey(lua_State *L){//regobj.openkey
	reg_aux_newkey(L, reg_aux_gethkey(L, 1),
		lua_checktstring(L, 2), NULL, reg_aux_getaccess2(L, 3), FALSE);
	return 1;
}
#ifndef LUA_REG_NO_HIVEOPS
//docok
